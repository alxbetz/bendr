...
##Fitting a single dose response curve to several replicates and compute prediction bands using the delta method
...
##Fitting a single dose response curve to several replicates and compute prediction bands with the bootstrap


```{r bootstrap_replifit,eval=FALSE}
rdata = as_tibble(
  data.frame(
    conc=c(0.1,1,10,100,1000,5000),
    replicateA=c(1,12,27,65,88,100),
...
fitObjectRep = fitdr_replicates(drc.formula,rdata,2:4,conc,ci_method = "bootstrap")
pRep = plot_single_drc(fitObjectRep)
pRep
```
